...
class Pancake extends React.Component {
  ...
  updateCounter = () => {    this.setState({      timeCooked: this.state.timeCooked + 1    })  }
  startInterval = () => {    this.interval = setInterval(this.updateCounter, 1000);  }
  cleanUpInterval = () => {    clearInterval(this.interval);  }
  flip = () => {    this.setState({      flippedAt: this.state.timeCooked    })  }
  getPancakeStatus = () => {
